...
class EmbeddingTE(tf.keras.layers.Layer):
    ...
    def __init__(
            ...
            *,
            ...
            backend=None,
            differentiator=None,
            initializer=tf.keras.initializers.RandomNormal(0, 2 * np.pi),
            regularizer=None,
            constraint=None,
            **kwargs,
    ):
        ...
        model_circuit = self.construct_circuit() + model_circuit
        self._actual_circuit = model_circuit
    ...
    def construct_circuit(self):
        qubits = cirq.GridQubit.rect(self._num_embedding, 1)
        circuit = cirq.Circuit()
        for i in range(self._num_embedding):
            symbol1 = sympy.Symbol('rx' + '-' + str(i))
            symbol2 = sympy.Symbol('ry' + '-' + str(i))
            circuit.append(cirq.rx(symbol1)(qubits[i]))
            circuit.append(cirq.ry(symbol2)(qubits[i]))
    ...
    def call(self, inputs):
        """
        Embedding Layer:
        input: N x 2(num_qubit) as theta, phi for each input

        Used after keras embedding layer 
        """
        circuit_batch_dim = tf.gather(tf.shape(inputs), 0)
        tiled_up_model = tf.tile(self._model_circuit, [circuit_batch_dim])
        model_appended = tiled_up_model #self._append_layer(inputs, append=tiled_up_model)
        tiled_up_parameters = tf.concat([inputs, tf.tile([self.parameters[self._num_embedding * 2:]], [circuit_batch_dim, 1])], axis=1)
        print(tiled_up_parameters)
